  ...
  Node parse(BlockParser parser) {
    ...
    while (!parser.isDone) {
      ...
      if (tryMatch(_emptyPattern)) {
        ...
        break;
      } else if (tryMatch(_checkPattern) ||
          tryMatch(_ulPattern) ||
          tryMatch(_olPattern)) {
        ...
        listMarker = marker;
        // 保存 check状态
        checked.add(marker == ' ' ? false : (marker == 'x' ? true : null));
        // TODO: marker length - [X]
    ...
    if (listTag == 'ol' && startNumber != 1) {
      ...
      // cl
      return Element(listTag, itemNodes)
        ..attributes['check'] = checked.toString();
    }
  }
...
/// Parses unordered lists.
class CheckBoxListSyntax extends ListSyntax {
  const CheckBoxListSyntax();

  @override
  RegExp get pattern => _checkPattern;

  @override
  String get listTag => 'cl';
}
