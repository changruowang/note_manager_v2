...
// import 'src/block_parser.dart';

// class BlockImage extends BlockSyntax {
//   @override
//   RegExp? get pattern => RegExp(r'\[.*\]\(.*\)');
//   @override
//   Node? parse(BlockParser parser) {
//     final match = RegExp(r'\(.*\)').firstMatch(parser.current);
//     if (match != null) {
//       if (match.start + 1 <= match.end - 1) {
//         var imgPath = parser.current.substring(match.start + 1, match.end - 1);
//         return Node();
//       }
...
class DeltaMarkdownDecoder extends Converter<String, String> {
  ...
  String convert(String input) {
    ...
    final markdownDocument = Document(blockSyntaxes: []).parseLines(lines);

    return jsonEncode(_DeltaVisitor().convert(markdownDocument).toJson());
  }
}

class _DeltaVisitor implements NodeVisitor {
  static final _blockTags =
      RegExp('h1|h2|h3|h4|h5|h6|hr|pre|cl|ul|ol|blockquote|p|pre');
  ...
  late Element previousToplevelElement;

  Delta convert(List<Node> nodes) {
    delta = Delta();
  ...
  bool visitElementBefore(Element element) {
    ...
    if (delta.isNotEmpty && _blockTags.firstMatch(element.tag) != null) {
      if (element.isToplevel) {
        ...
        if (previousToplevelElement.tag != 'ul' &&
            ...
            previousToplevelElement.tag != 'cl' &&
    ...
    if (_embedTags.firstMatch(element.tag) != null) {
      if (element.tag == 'img') {
        delta.insert(getImageAttribute(element.attributes['src']!));
      } else {
  ...
  Attribute? _tagToAttribute(Element el) {
    switch (el.tag) {
      case 'em':
      ...
      case 'cl':
        return jsonDecode(el.attributes['check']!)[0]
            ? Attribute.checked
            : Attribute.unchecked;
      case 'ul':
        return Attribute.ul;
...
Map<String, dynamic> getImageAttribute(String path) {
  return {'_type': 'image', 'image': path, '_inline': false};
}
